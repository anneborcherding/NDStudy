...
def getAllThresholds(app=None):
    ...
    for thresholdSetItem in THRESHOLD_SETS:
        ...
        for dataSetItem in thresholdSet['dataSet']:
            ...
            dataNameStr = "_".join(dataName)
            if dataNameStr in stat_data:
                thresholds[(dataSet, dataNameStr, thresholdSet['name'])] = buildThreshold(stat_data[dataNameStr],
                                                                                          thresholdSet)
...
def runBestCrawls(apps, host):
    if apps:
        APPS = apps
    BASE_COMMAND = ['java', '-jar',
                    '/art-fork/crawljax/examples/target/crawljax-examples-3.7-SNAPSHOT-jar-with-dependencies.jar']
    RUNTIME = 30
    succesful = []
    unsuccesful = []
    skipped = []
    ...
    for app in APPS:
        ...
        for algo in ALGOS:
            ...
            for threshold in safThresholds:
                ...
                logFile = os.path.join("logs", "crawljaxLog_" + app + "_" + algoStr + "_" + str(RUNTIME) + "_" + str(
                    threshold) + "_" + str(datetime.now().strftime("%Y%m%d-%H%M%S") + ".log"))

                done, command = runAlgo(appName=app, algo=algoStr, runtime=RUNTIME, threshold=threshold, host=host, logFile=None, maxStates=150)
...
def runAllAlgos(apps=APPS, host="localhost"):
    if apps:
        APPS = apps
    ...
    for app in APPS:
        ...
        for algo in ALGOS:
            ...
            for threshold in safThresholds:
                ...
                done, command = runAlgo(appName=app, algo=algoStr, runtime=RUNTIME, host=host, threshold=threshold, logFile=None)
...
def runAlgo(appName, algo, runtime, logFile, host="localhost", threshold=-1,
            maxStates=-1, rerun=False):
    ...
    crawljaxOutputPath = os.path.join(os.path.abspath(os.path.join("..", "out", appName, crawlFolderName, host)), "")

    if not logFile:
        # logfile = os.path.join("logs", "crawljaxLog_" + str(datetime.now().strftime("%Y%m%d-%H%M%S")) + ".log"
        logFile = os.path.join(crawljaxOutputPath, "crawljaxLog_" + str(datetime.now().strftime("%Y%m%d-%H%M%S")) + ".log")
    ...
    print(f"Crawljax Output Path: {crawljaxOutputPath}")
    ...
    if not rerun:
        if os.path.exists(crawljaxOutputPath):
            if len(existingValidCrawls) == 0:
                ...
                Path(crawljaxOutputPath).mkdir(parents=True, exist_ok=True)
            else:
...
if __name__ == "__main__":
    BASE_COMMAND = ['java', '-jar',
                    '../crawljax/examples/target/crawljax-examples-3.7-SNAPSHOT-jar-with-dependencies.jar']
    RUN_EXTERNAL = True

    target_host = "192.168.100.121"
    app = "http://192.168.100.121"

    DRY_RUN = False

    # testRestartDocker()
    if RUN_EXTERNAL:
        # runBestCrawls(apps=[app], host=target_host)
        runAllAlgos(apps=[app], host=target_host)
    else:
        # runBestCrawls()
        runAllAlgos()
